...
def ensure_project_state_table():
    db = get_db()
    db.executescript(
        """
        CREATE TABLE IF NOT EXISTS project_state (
...
# 👇 add this one so router can always read it
...
# ───────────────────────── MAIN APP ───────────────────────── 
    ...
    # ─── ROUTER: if a project is “open”, show its dedicated tool ───
    if st.session_state.get("current_project"):
        pid   = st.session_state.current_project
        ptype = st.session_state.current_type   # "Promo Analysis", "Category Forecasting", "MMM"

        module_for = {
            "Promo Analysis":        "promo_tool",
            "Category Forecasting":  "cat_forecast_tool",
            "MMM":                   "mmm_tool",
        }
        mod_name = module_for.get(ptype)
        if mod_name is None:
            st.error(f"Unknown project type: {ptype}")
        else:
            tool = importlib.import_module(mod_name)
            tool.run(pid)            # 🔻 launch the right mini‑app
        st.stop()                    # nothing below runs while inside a tool
                ...
                with col:
                    ...
                    with bcol1:
                        if st.button("🔍 View", key=f"view_{pid}", use_container_width=True):
                            st.session_state.current_project = pid    # store id, not name
                            st.session_state.current_type    = proj_type
                            st.rerun()
